...
/** 
* @param {org.unblockcerti.user.certificate.addCerti} certiData
* @transaction
*/

function addCerti(certiData){
    return getAssetRegistry('org.unblockcerti.user.certificate.Certificate')
    .then(function(res){
        var factory = getFactory();
        var  NS =  'org.unblockcerti.user.certificate';
        var  certiId = certiData.certiId;
        var  app = factory.newResource(NS,'Certificate',certiId);
        app.applicationId = certiData.applicationId;
        app.field1 = certiData.field1;
        app.field2 = certiData.field2;
        return res.add(app);
    });
}
